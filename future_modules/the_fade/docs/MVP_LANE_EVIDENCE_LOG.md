...
- **observe:** reads one real **HTTPS URL** or **local file path** Ã?Æ?Ã?Â¢Ã?Â¢Ã¢â??Â¬Ã?Â Ã?Â¢Ã¢â??Â¬Ã¢â??Â¢ writes `future_modules/the_fade/outputs/lane_b_real_observation/{task_id}_scout_failure.json` **or** `{task_id}_normalized_signal_event.json` (stdout prints the same object).
- **conflict:** reads one **lane B artifact** (normalized JSON from observe) + one **THE FADE-local** `context_only_contra` JSON (`semantic_role` = `lane_e_research_swarm_context`) Ã?Æ?Ã?Â¢Ã?Â¢Ã¢â??Â¬Ã?Â Ã?Â¢Ã¢â??Â¬Ã¢â??Â¢ writes `{task_id}_conflict_packet.json`; uses `config/fusion_policy.json` **read-only** for weight wording.
...
**Protocol reference:** `docs/MVP_SOURCE_RELIABILITY_AUDIT.md` Ã?Æ?Ã?Â¢Ã?Â¢Ã¢â??Â¬Ã?Â Ã?Â¢Ã¢â??Â¬Ã¢â??Â¢ **Lane B pre-audit reliability window protocol (Tranche 21)** (stricter target -- **preserved**; **not** erased).
...
### Full 24-attempt schedule (UTC) -- `window_start_utc + (i Ã?Æ?Ã?â??Ã?Â¢Ã¢â??Â¬Ã¢â?¬Â 2h)` for `i = 0..23`
