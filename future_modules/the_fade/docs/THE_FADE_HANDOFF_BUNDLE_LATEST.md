...
## New chat checkpoint (post–T115 run-scoped source-record capture governance)

- **Where you are:** Phase **2** for **binding approval**; **post–T115** governance (Prompt **#413**). **Scoped MVP approval** (**T80**) remains: `approved: true`. **T114** **`build_operator_attention_queue.py`** + **`operator_attention_queue.json`** remain on disk. **T115** governs **exactly one** future implementation tranche: per-run **`source_records_snapshot.json`** via **one** bounded Federal Register GET (**`build_source_records_snapshot_from_run_folder.py`** — **not** written in **T115**).
- **What to do next:** Read **`THE_FADE_CONTEXT_ANCHOR.md`** and **`THE_FADE_T115_RUN_SCOPED_SOURCE_RECORD_CAPTURE_GOVERNANCE.md`**. Do not implement capture without a prompt that explicitly executes the **T115**-scoped build; do not widen into scanner/runtime/dashboard without new governance.
...
| T82 status                   | **Executed** — Post-T80 Phase 3 entry governance lock (**#342**); **`phase2_mvp_approval_scope_decision.json`** **`post_t82_on_disk_governance_lock`** + **`phase3_entry_planning_only_definition`** + amended **`next_tranche_authorization_rule`**; **no** code; **no** lane reopen; **planning-only** next class **allowed**; **implementation** **forbidden** until later governance |
| T83 status                   | **Executed** — Phase 3 entry planning lock (**#345**); **`THE_FADE_PHASE3_ENTRY_PLAN_T83.md`** + **`THE_FADE_PHASE3_IMPLEMENTATION_BOUNDARY_T83.md`**; **`post_t83_phase3_entry_planning_package`**; **planning only**; **no** implementation; **no** lane reopen; **no** code |
...
| T97 status                   | **Executed (governance only)** — Local end-to-end happy-path runner slice (**#381**); **`THE_FADE_T97_LOCAL_END_TO_END_HAPPY_PATH_RUNNER_GOVERNANCE.md`** + **`THE_FADE_T97_LOCAL_END_TO_END_HAPPY_PATH_RUNNER_ACCEPTANCE_CRITERIA.md`**; **`post_t97_local_end_to_end_happy_path_runner_governance`**; authorized one future runner tranche only |
| T98 status                   | **Executed (implementation)** — Local end-to-end happy-path runner build (**#384**); **`runner/local_happy_path/run_local_end_to_end_happy_path.py`**; **`post_t98_local_end_to_end_happy_path_runner_build`**; local sequential orchestration only; **not** scanner/runtime/provider/dashboard |
| T107 status                  | **Executed (governance only)** — Operator review gate signal (**#399**); **`THE_FADE_T107_OPERATOR_REVIEW_GATE_SIGNAL_GOVERNANCE.md`** + **`THE_FADE_T107_OPERATOR_REVIEW_GATE_SIGNAL_ACCEPTANCE_CRITERIA.md`**; **`post_t107_operator_review_gate_signal_governance`** |
