#!/usr/bin/env python3
"""Offline validator for ingress-written UniverseScannerRequest JSON packets.

Scope boundary (T91):
- Reads only inputs/phase3_universe_scanner_requests/ (root *.json + optional fixtures_invalid/)
  and schemas/universe_scanner_request.schema.json under this contract package.
- Emits stdout/stderr and process exit code only.
- No network, scanner, result generation, or writes to inputs/contracts.
"""
...
REQUEST_SCHEMA_PATH = _BASE / "schemas" / "universe_scanner_request.schema.json"
INPUTS_DIR = _THE_FADE_ROOT / "inputs" / "phase3_universe_scanner_requests"
FIXTURES_INVALID_DIR = INPUTS_DIR / "fixtures_invalid"
...
def _infer_fixture_reason(path: Path) -> str:
    """Derive expected jsonschema validator keyword from filename (T90/T87-style tokens)."""
    name = path.stem.lower()
    if "_request_" not in name:
        raise ValueError(
            f"{path.name}: fixture name must include '_request_' (request-only ingress scope)."
        )
    if "_type_" in name:
        return "type"
...
def _validate_root_packets(validator: Draft7Validator) -> List[Tuple[bool, str]]:
    if not INPUTS_DIR.is_dir():
        return [(False, f"FAIL: missing inputs directory: {INPUTS_DIR}")]

    paths = sorted(INPUTS_DIR.glob("*.json"))
    if not paths:
        return [(False, "FAIL: no root-level *.json under inputs/phase3_universe_scanner_requests/")]
    ...
    for path in paths:
        ...
        if errors:
            ...
            outcomes.append((True, f"{path.name}: PASS (valid against request schema)"))
    return outcomes
...
def main() -> int:
    if not REQUEST_SCHEMA_PATH.exists():
        print(f"FAIL: missing schema: {REQUEST_SCHEMA_PATH}", file=sys.stderr)
        return 1

    validator = _build_validator(REQUEST_SCHEMA_PATH)
