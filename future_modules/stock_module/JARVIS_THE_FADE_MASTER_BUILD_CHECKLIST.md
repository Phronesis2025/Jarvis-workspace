...
| Active lane focus | **None by default** — **post–T76** (**#329**): **`lane_b_t76_scoped_mvp_governance_acceptance_pack.json`** + memo — **subset** acceptance **established** for **four** dimensions; **`stale_outage_behavior` not** accepted; **`primary_lane_eligibility_met` remains false**; **no** approval flip authorized. **`mvp_lane_approval.json`** **`approved: false`**. **No** lane **automatically active** |
...
| Phase 2 remaining gate plan (Prompt **#134** / Tranche **34A**) | **LOCKED:** `future_modules/the_fade/docs/THE_FADE_PHASE2_REMAINING_GATE_PLAN.md` — **through T76 executed on disk** (**T76** governance acceptance pack); Phase **2** still active; **not** approval; **not** Phase 3 |
