...
- [x] Module Checklists dashboard lane — **complete enough for current phase**; canonical checklist state created (`state/module_checklists.json`, `state/MODULE_CHECKLISTS.md`); dashboard page at `/checklists` added; nav includes Checklists entry; page reads canonical JSON and renders read-only (module name, purpose, status, current phase/step, final version, phase-by-phase checklist, done vs remaining counts); local dev rendering verified (Overview and Checklists both render with correct shell/styling when `npm run dev` on port 3001); build/start proof separate and still blocked on Windows (EPERM)
