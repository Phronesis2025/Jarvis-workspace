...
- Jarvis Dashboard v1 is now live on Vercel as a read-only Supabase-backed dashboard. Local JSON/Markdown files remain the source of truth; Supabase is used only as a dashboard read model. The manual exporter (`export_dashboard_data.py`) is working and the dashboard is reading real data from Supabase. The exporter now populates WCS trust metrics from local evidence: build status, smoke status, page-smoke status, page route when applicable, stop reason, and operator_checkpoints (manual_check, screenshot). Overview shows a compact "Latest WCS run trust" section; Recent Runs exposes trust signals (B/S/P) for WCS rows. **Overview now surfaces B1 module status** (bounded, read-only, first site, proof summary) **and a separate B1 process chart** (Watcher Config → Route Checks → Evidence Capture → Noise Filter/Dedupe → Proposed Defect Packets → Operator Review). **Exporter health is visible in Overview** as a compact status box (dry-run available, dry-run proof, live export separate proof surface). **Module Checklists page live (2026-03-20):** dedicated `/checklists` page backed by canonical `state/module_checklists.json`; read-only; shows per-module build-path (purpose, status, current phase/step, final version, phase-by-phase checklist, done vs remaining counts); nav includes Checklists entry; local dev rendering verified (Overview and Checklists both render with correct shell/styling when `npm run dev` on port 3001). **Stale Overview data issue fixed (2026-03-20):** app-side fetch caching was causing Overview to show stale values; `fetchCache = "force-no-store"` added on the Overview route; after restart and export refresh, Overview now reads live exported data (Last dashboard update, What happened today, Recent activity, WCS task totals). **Overview is now trustworthy enough for live workflow monitoring** after restart and export refresh. No write-back editing, command execution, scheduling controls, or automatic sync are live in the dashboard. No new Supabase-backed B1 data model; B1 is surfaced on Overview only; no dedicated B1 page or nav item yet. **Dashboard verification:** lint passes; build remains environment-blocked on some Windows setups (EPERM on Next trace file). Exporter supports `--dry-run` for payload/env health without writes; live export connectivity/write success remains a separate proof surface.
...
Every new Jarvis chat must include:
