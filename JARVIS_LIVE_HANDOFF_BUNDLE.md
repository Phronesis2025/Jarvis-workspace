...
- Last updated: 2026-03-18 (Dashboard v1 live deployment recorded)
...
- Jarvis Dashboard v1 is now live as a read-only Supabase-backed dashboard deployed on Vercel. Local JSON/Markdown files remain the source of truth; Supabase is used only as a dashboard read model. Dashboard data is currently populated by the manual export script (`export_dashboard_data.py`). No write-back editing, command execution, scheduling controls, or automatic sync are live in the dashboard.
