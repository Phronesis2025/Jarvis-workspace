...
- Jarvis Dashboard v1 is now live on Vercel as a read-only Supabase-backed dashboard. Local JSON/Markdown files remain the source of truth; Supabase is used only as a dashboard read model. The manual exporter (`export_dashboard_data.py`) is working and the dashboard is reading real data from Supabase. The exporter now populates WCS trust metrics from local evidence: build status, smoke status, page-smoke status, page route when applicable, stop reason, and operator_checkpoints (manual_check, screenshot). Overview shows a compact "Latest WCS run trust" section; Recent Runs exposes trust signals (B/S/P) for WCS rows. No write-back editing, command execution, scheduling controls, or automatic sync are live in the dashboard. **Dashboard verification hardening:** lint now runs non-interactively and passes; build remains environment-blocked on some Windows setups (EPERM on Next trace file). Exporter now supports `--dry-run`: loads env, gathers local data, prints summary; does not import Supabase or perform writes. Dry-run verifies payload/env health without writes; live export connectivity/write success remains a separate proof surface.
...
- **Full-system sweep 2026-03-19:** WCS verification passed (build, base smoke, page-smoke `/schedules`, dev server healthy). Pathfinder verification passed (no-LLM, LLM-enabled, result validation). Dashboard lint fixed and passes; dashboard build remains environment-blocked on Windows (EPERM on Next trace file). Exporter now has `--dry-run` for safe verification (payload/env health without writes); live export connectivity/write success remains a separate proof surface. B1 v1 Local Website Defect Watcher implemented and proven; fresh proof run 0 findings after signal hardening. Overall verdict: mostly passed with bounded blockers.
