...
- Last updated: 2026-04-15 (dashboard UI transplant tranche lock + dashboard reliability fixes; #558)
- Status: active live handoff bundle for current Jarvis hardening state

## Hybrid operating doctrine (v1, locked 2026-04-10)

- **Location:** `JARVIS_HYBRID_OPERATING_DOCTRINE_v1.md` (repo root).
- **Prompt IDs (locked, from #500):** Every Cursor/project prompt must carry a **non-blank global sequential number**, plus `PROMPT:` / `LABEL:` / `TRACK:` lines per **§12** in the doctrine. **#500** is the first official number; **#501** is next. Tracks: **MAIN_BUILD**, **SIDE_QUEST**, **GOVERNANCE**, **PROOF**, **DOC_WAVE** (definitions in doctrine).
- **Summary:** The **deterministic spine** remains the **sole authoritative writer** for loop truth (`state/`, governed `tasks/` / `results/` / `qa/`). **GPT** = planner/reviewer only (no direct workspace writes). **Cursor Auto** = default builder; **Cursor Premium** = hard implementation/debugging only. **Ollama** = local low-cost cognition for **non-authoritative** drafts under `scratch/model_drafts/` only. **Promotion** of any model draft into truth is **operator-mediated** or **deterministic-script-only** (no models inside spine writers in v1).
- **Doc wave 1:** Core v3 docs, script reference, task checklist, `module_checklists.json`, `file_registry.json`, structure doc, this note, canonical `JARVIS_NEW_CHAT_HANDOFF_BUNDLE_LATEST.md` pointer, `.gitignore` exception for that file—**no** THE FADE governance edits, Pathfinder activation, Research Swarm pipeline rewiring, dashboard write-back, or local-worker implementation scripts.
- **Tranche 1 — minimal local draft lane (operator proof, 2026-04-11):** On the operator machine, `future_modules/local_llm_worker/scripts/ollama_health_check.py` returned **PASS** against local Ollama; `qwen2.5:7b` (per `future_modules/local_llm_worker/config/local_model_config.json`) ran **`run_local_summary.py`** successfully and produced a **non-authoritative** summary file under `scratch/model_drafts/`. Summary text was **subjectively cleaner** after the prompt-tightening pass. **Not proven:** CI automation, other models, or any **authoritative** integration (no `jarvis.py`, reconcile, validators, packets, or dashboard). Scope remains **text-file summary only** for this tranche.
- **Tranche 2 — draft JSON extraction (operator proof, 2026-04-12):** **`run_local_extract_json.py`** ran successfully on a **sample input**; a **schema-validated** JSON draft was written under **`scratch/model_drafts/`** (e.g. `*_extract.json`) with keys **source_path**, **timestamp_utc**, **non_authoritative**, **main_points**, **actionable_items**, **entities_mentioned**. **Not proven:** CI, batch extraction, semantic accuracy guarantees, or any **authoritative** use of extracted JSON. Scope remains **draft-only extraction** with no spine integration.
- **Tranche 3 — draft JSON classification (operator proof, 2026-04-13):** **`run_local_classify_json.py`** ran successfully on a **sample input**; a **schema-validated** JSON draft was written under **`scratch/model_drafts/`** (e.g. `*_classify.json`) with keys **source_path**, **timestamp_utc**, **non_authoritative**, **primary_topic**, **content_type**, **usefulness_guess**, **reasoning_short**. The script **rejects** invalid **`content_type`** and **`usefulness_guess`** values before write (single-sample proof only). **Not proven:** CI, batch classification, quality across diverse inputs, other models, or any **authoritative** integration. **No** authoritative state wiring exists for this lane. Scope remains **draft-only classification**.
- **Research Swarm:** Canonical RS new-chat doc remains **`RESEARCH_SWARM_NEW_CHAT_HANDOFF_BUNDLE_LATEST.md`** for RS-only chats (not deduplicated in this wave).
- **Analyst Failure Taxonomy v1 (2026-04-14, manual-first):** Governed failure-first review framework for analyst-style outputs (X, GitHub, Research Swarm QA, source comparisons). Canonical: `future_modules/research_swarm/docs/JARVIS_ANALYST_FAILURE_TAXONOMY_V1.md` + `future_modules/research_swarm/config/jarvis_analyst_failure_taxonomy_v1.json`. Explicitly **not** for WCS UI QA, lint/build, route smoke, trading PnL, or portfolio risk. **Not** automated scoring or dashboard chips until proven in real reviews.
- **WCS issue scanner tranche 1 proof locked (2026-04-14):** `scripts/run_wcs_issue_scan.py` with `scripts/wcs_issue_scan_allowlist.json` and `scripts/wcs_issue_scan_playwright.cjs` — allowlist-only, no crawl; writes `scratch/issue_scan_candidates/<run_id>/` (`candidates.json`, `scan_meta.json`, `summary.md`, per-route evidence). Example run `20260414T230751Z` completed with exit 0 and structured non-authoritative candidates. **Not proven:** promotion to backlog, full-site coverage, or parity with formal smoke/QA.
- **WCS candidate promotion tranche 1 proof locked (2026-04-15):** `scripts/run_wcs_candidate_promotion_packet.py` reads a prior scan run (`scratch/issue_scan_candidates/<scan_run_id>/`) and writes one bundle under `scratch/candidate_promotion_packets/<bundle_id>/` containing `promotion_review.md` + `intake_payload.json`. Example bundle: `20260414T230751Z_promo_20260415T025919Z`, exit 0, proposed_items=4. **Not proven:** authoritative backlog merge, task creation, or automated promotion.
- **WCS manual merge-aid tranche 1 proof locked (2026-04-15):** `scripts/run_wcs_promotion_packet_merge_aid.py` reads an existing promotion packet (`scratch/candidate_promotion_packets/<bundle_id>/`: `intake_payload.json`, `promotion_review.md`) and writes one scratch report bundle (`scratch/candidate_merge_aid_reports/<report_id>/`: `merge_aid_report.json`, `merge_aid_report.md`). One real run: bundle `20260414T230751Z_promo_20260415T025919Z` → report `merge_aid_20260415T040203Z`, exit 0, disposition `ready_for_manual_merge_review`, `total_items=4` / `valid_items=4` / `flagged_items=0`. Outputs are scratch-only and non-authoritative. **Not proven:** authoritative backlog merge, task creation, automated promotion, or any write to governed backlog/task surfaces.
- **WCS manual promotion procedure v1 (2026-04-15, #544):** Canonical operator-only steps for when (and when not) to edit authoritative backlog after the scratch chain: `scripts/WCS_MANUAL_PROMOTION_PROCEDURE_V1.md`. **Human-only spine edits in v1**; no automated backlog writer added. **Not in scope:** auto task creation, scanner/merge-aid changes, dashboard, or LLM promotion.
- **WCS manual promotion procedure single-item proof locked (2026-04-15, #546):** #545 followed `scripts/WCS_MANUAL_PROMOTION_PROCEDURE_V1.md` and promoted exactly one reviewed candidate from the proven chain into authoritative backlog truth (`WCS-062` from `candidate_id=20260414T230751Z_0003_abd1bb`), with manual `state/master_backlog.json` update, `state/MASTER_BACKLOG.md` regeneration, and `state/DAILY_REVIEW.md` audit note including `scan_run_id`, `bundle_id`, `report_id`, `candidate_id`, and evidence path traceability. No `tasks/**`, `results/**`, or `qa/**` artifacts were created in this promotion step. **Not proven:** bulk promotion or automated backlog writing.
- **WCS promotion preflight validator proof locked (2026-04-15, #548):** `scripts/run_wcs_promotion_preflight_check.py` is proven as a read-only/manual-assist checker that reads scan/promotion/merge-aid/manual-procedure artifacts and writes one scratch preflight bundle (`preflight_report.json` + `preflight_report.md`) under `scratch/candidate_preflight_reports/<preflight_report_id>/`. Real run for `candidate_id=20260414T230751Z_0003_abd1bb` returned exit 0 with disposition `ready_for_manual_promotion`. **Not proven:** authoritative backlog/task creation, bulk promotion, or automated backlog writing.
- **WCS promotion session preflight wrapper proof locked (2026-04-15, #550):** `scripts/run_wcs_promotion_session_preflight.py` is proven as a read-only/manual-assist multi-candidate session checker that reads scan/promotion/merge-aid/manual-procedure artifacts and writes one scratch session bundle (`session_preflight_report.json` + `session_preflight_report.md`) under `scratch/candidate_preflight_sessions/<session_report_id>/`. Real run for `candidate_id=20260414T230751Z_0003_abd1bb` and `candidate_id=20260414T230751Z_0002_162ec6` returned exit 0 with overall disposition `all_ready_for_manual_promotion` and deterministic per-candidate dispositions. **Not proven:** authoritative backlog/task creation, bulk promotion into truth, or automated backlog writing.
- **WCS promotion session summary helper proof locked (2026-04-15, #552):** `scripts/run_wcs_promotion_session_summary.py` is proven as a read-only/manual-assist summarizer that reads an existing session preflight report and writes one scratch summary bundle (`session_decision_summary.json` + `session_decision_summary.md`) under `scratch/candidate_preflight_summaries/<summary_id>/`. Real run for source `preflight_session_20260415T125714Z` returned exit 0 with overall recommendation `safe_to_review_for_promotion` and deterministic per-candidate decision buckets. **Not proven:** authoritative backlog/task creation, bulk promotion into truth, or automated backlog writing.
- **WCS-062 governance resolution + promotion hygiene proof locked (2026-04-15, #557):** Follow-up investigation after governed retry showed `WCS-062` is **not a clean repo-owned actionable defect** in current bounded scope, so authoritative truth was updated honestly to **`deferred`** (not executed/successfully fixed). Traceability was retained while removing scope-polluting evidence-file tokens from backlog notes. `scripts/WCS_MANUAL_PROMOTION_PROCEDURE_V1.md` now explicitly requires separation of executable scope tokens (repo paths) from traceability tokens (IDs/evidence directory anchors) to prevent future evidence-path-to-scope contamination. **Not proven:** scanner-wide false-positive accuracy or automated false-positive handling.
- **Footer email hydration caret-noise guard proof locked (2026-04-15, #568/#569):** Narrow governance pattern from deferred **WCS-062** / **WCS-063** is encoded in `scripts/wcs_footer_hydration_caret_noise.py`. **Merge-aid** tags matching intake text per item (`footer_email_hydration_caret_noise_suspected`) without claiming scanner-wide accuracy or changing merge-aid disposition when structure is valid. **Single-candidate preflight** downgrades that pattern to **`needs_operator_cleanup`** (`footer_email_hydration_caret_noise_requires_operator_review`). **Session preflight** applies the same guard. **Manual promotion procedure** forbids treating this pattern as a normal promotable repo-owned defect without stronger evidence. Bounded proof: example bundle `20260414T230751Z_promo_20260415T025919Z` post-guard merge-aid `total=4` / `valid=0` / `flagged=4` / disposition `ready_for_manual_merge_review`; example preflight `20260414T230751Z_0002_162ec6` → `needs_operator_cleanup`. **Not proven:** general scanner false-positive handling beyond this pattern, or any change to authoritative backlog automation.
- **Packet-scope derivation hardening proof locked (2026-04-15, #559):** `scripts/jarvis_task_packet_scope.py` was hardened so executable scope derivation rejects traceability/evidence pollution tokens (`evidence=...`, `scratch/**`, metadata-like token fragments, and arbitrary workspace evidence-file paths), while preserving valid repo-relative execution scope paths. Bounded proofs covered both negative rejection and positive acceptance, including mixed-input behavior where invalid tokens are dropped while valid repo scope remains. **Not proven:** scanner-wide accuracy, promotion correctness in general, or broader backlog correctness guarantees.
- **Packet-scope end-to-end regression proof locked (2026-04-15, #561):** Bounded end-to-end proof through the real packet/gate path confirms consistency after hardening: (1) `generate_task_packet` rejects malformed evidence-style notes as executable scope; (2) `worker_change_check` and `commit_gate_check` agree on that negative rejection path; (3) `generate_task_packet` preserves a valid repo-relative path while dropping bad evidence tokens; and (4) both gates agree on that positive path as derived expected scope. **Not proven:** scanner accuracy in general, backlog correctness in general, or full task execution correctness.
- **WCS repo-local `CLAUDE.md` (2026-04-15):** Bounded AI-assisted coding discipline at `C:\dev\wcsv2.0-new\CLAUDE.md` (think/simplicity/surgical/goal-driven; packet scope; verification commands). Reinforces Jarvis norms; **does not** replace task packets, QA, or operator review.
...
- Pathfinder v1 is now proven as a bounded read-only intake/investigation worker for WCS. Proof was completed with a successful live run of `run_pathfinder.py` using a realistic WCS intake packet. Pathfinder accepts minimal intake or full task-packet input, gathers bounded context from workspace artifacts and WCS repo files, and produces structured result output with an optional draft backlog candidate. Optional LLM synthesis is implemented via `pathfinder_llm_synthesis.py`; when the module is unavailable, the API key is missing, or synthesis fails, Pathfinder safely falls back to rule-based output and records `llm_skipped_reason`. Result output includes `synthesis_source` (rule_based or llm) and, when applicable, `llm_skipped_reason`. CLI flags: `--no-llm` (skip LLM), `--model` (override model). **Fallback + LLM path proven 2026-03-19:** fallback proof: `--no-llm` => `rule_based`/`no_llm`; earlier LLM-enabled failure: `rule_based`/`validation_failure`; current LLM-enabled success: `synthesis_source=llm`. Validation failure diagnostics now preserve specific reason text via `validation_failure:<reason>`. It remains manual/CLI-invoked, read-only, and intentionally limited: no code edits, no git actions, no unattended mode, no scheduling, and no broad repo crawl.
- Jarvis Dashboard v1 is now live on Vercel as a read-only Supabase-backed dashboard. Local JSON/Markdown files remain the source of truth; Supabase is used only as a dashboard read model. The manual exporter (`export_dashboard_data.py`) is working and the dashboard is reading real data from Supabase. The exporter now populates WCS trust metrics from local evidence: build status, smoke status, page-smoke status, page route when applicable, stop reason, and operator_checkpoints (manual_check, screenshot). The completed dashboard UI transplant tranche is now locked: **shell transplant complete** (sidebar + topbar while preserving current routes and Foundry access), **home route upgraded into the Jarvis Now surface**, **`/tasks` upgraded into the Jarvis Work surface**, and **`/runs` upgraded into the Jarvis execution/audit surface**. **Lovable_UI was used as a visual/UI donor/reference only**; the production dashboard remains the existing Next.js app with real routes, real data, and preserved backend behavior. **No new Lovable-only routes** such as `/modules`, `/history`, or `/rules` were added, and no backend/task-model rewrite occurred. **Visual system alignment** toward the Lovable token system is in place inside the real dashboard only. **Reliability fixes required and locked:** (1) `tsconfig.json` excludes `Lovable_UI`, `_snapshots`, and build artifacts from the real dashboard TypeScript/build scope; (2) `next.config.js` now separates dev/prod Next output directories so `npm run build` does not break route CSS assets for `npm run dev`. **Module Checklists page live (2026-03-20):** dedicated `/checklists` page backed by canonical `state/module_checklists.json`; read-only; shows per-module build-path (purpose, status, current phase/step, final version, phase-by-phase checklist, done vs remaining counts); nav includes Checklists entry. **Dashboard verification:** lint passes; build passes; major routes have been rechecked with CSS assets loading correctly after the dev/prod output-dir separation. No write-back editing, command execution, scheduling controls, or automatic sync are live in the dashboard. No new Supabase-backed B1 data model; B1 is surfaced on the Now/Overview surface only; no dedicated B1 page or nav item yet. Remaining dashboard surfaces can be migrated later if desired, but this tranche is intentionally locked.
- **B1 v1 Local Website Defect Watcher** is now a runnable bounded module. First monitored site: https://www.wcsbasketball.site/. Uses Playwright. CLI: `python scripts/run_local_website_defect_watcher.py --config <path>`. Read-only; writes run_result.json, screenshots, console_errors.json, evidence_manifest.json, proposed_defect_packets under scratch/local_website_defect_watcher/<watcher_id>/<run_id>/. Dedupe: state/local_website_defect_watcher_dedupe.json. v1 signal hardening complete: Supabase/WebSocket/realtime console noise filtered; example config selectors verified against live WCS site; spec matches actual behavior (direct route reachability, no nav-link clicking). First run exposed false positives; signal hardening corrected config/spec/noise; fresh proof run passed with 0 findings, 0 proposed packets, 4 screenshots, 0 console errors, recommended_action=dismiss. B1 is separate from WCS coding-worker flow and Pathfinder. Operator approval workflow not implemented; watcher creates proposed packets only.
...
- The operator-gated sequential runner is now proven for multiple tasks back-to-back in one live session. **5-task fake-task batch proof passed (2026-03-20):** all 5 tasks (WCS-037, WCS-038, WCS-039, WCS-047, WCS-049) completed through reconcile via `run_task_sequence.py`; live export ran after each completed task and passed; successful runs return WCS to clean `main`. Earlier proofs: WCS-028 (one-task), WCS-029 + WCS-030 (multi-task). The system remains sequential only. No scheduling, unattended mode, concurrency, or session persistence. Page-specific smoke support is live in WCS and was exercised on non-home routes during the 5-task proof. Blocked/excluded fake proof specimens (e.g. WCS-033, WCS-045 already satisfied) should not be treated as workflow failures; they are task-selection hygiene artifacts.
- **Operator-triggered tranche runner proof locked (2026-04-14):** `scripts/run_wcs_tranche_runner.py` is proven for (a) dry-run selection from authoritative backlog truth, (b) truthful `prep_handoff` boundary that stops before worker/post, and (c) truthful resumed `post_only` boundary for a prepped+implemented task when operator-supplied `--draft-worker-result` evidence is forwarded in the same invocation. Stop-on-first-hard-failure behavior is proven. Post-only resume no longer needs manual backlog-status hacks after the resume-contract fix. This remains operator-triggered only; no scheduling/unattended execution is proven.
- **First guarded scheduled tranche build (2026-04-14):** Added narrow Windows scheduling wrapper `scripts/run_wcs_guarded_scheduled_tranche_once.ps1` that runs only `prep_handoff` (`max-tasks 1`) via existing tranche runner command, with lock/no-overlap, kill switch file, and per-run audit artifacts under `scratch/scheduled_tranche_runs/`. Added registration helper `scripts/register_wcs_guarded_tranche_hourly_task.ps1` (hourly, disabled-by-default unless explicitly enabled). Scope is intentionally limited to scheduled prep dispatch only; no scheduled post-only, no unattended implementation/QA/reconcile completion.
- **Guarded scheduled prep_handoff tranche proof locked (2026-04-14):** Wrapper proven for normal PASS, lock-active safe skip, and kill-switch safe skip; timestamped `audit.json` + stdout/stderr per run. Task Scheduler registration helper exists and remains disabled-by-default unless explicitly enabled. **Not proven:** unattended worker/post/reconcile, continuous unattended hourly production, or broader scheduling beyond this prep_handoff-only slice.
- **Windows Task Scheduler launch proof locked (2026-04-14):** One real scheduled launch on this machine: task `JarvisWcsGuardedPrepHandoffHourly` registered via `scripts/register_wcs_guarded_tranche_hourly_task.ps1` (helper parses; `param` before `$ErrorActionPreference`), then `Start-ScheduledTask` after enable; run produced `scratch/scheduled_tranche_runs/20260414T210400Z/` with `audit.json` showing prep_handoff pass, exit 0. Task left **disabled** afterward. **Not proven:** continuous hourly operation in production, `post_only` via scheduler, or unattended implementation/reconcile.
- Pathfinder v1 is now proven. Proof was completed with a successful live run of `run_pathfinder.py --packet future_modules/pathfinder/examples/pathfinder_intake.example.json`. Pathfinder accepts minimal intake or full task packet, gathers bounded context from workspace artifacts and WCS repo suspected files, and produces structured result output (synthesis, findings, optional draft backlog candidate) or escalation when blocked. Optional LLM synthesis via `pathfinder_llm_synthesis.py`; safe fallback to rule-based when module/API key absent or synthesis fails; result includes `synthesis_source` and `llm_skipped_reason` when applicable. **Fallback + LLM path proven 2026-03-19.** Pathfinder confidence normalization hardening complete; stable enough for broader testing. Validation failure diagnostics preserve specific reason via `validation_failure:<reason>`. Read-only only; no code edits, no git actions, no unattended mode, no scheduling, no broad repo crawl.
- **Full-system sweep 2026-03-19:** WCS verification passed (build, base smoke, page-smoke `/schedules`, dev server healthy). Pathfinder verification passed (no-LLM, LLM-enabled, result validation). Dashboard lint passes; dashboard build remains environment-blocked on Windows (EPERM on Next trace file). Dashboard Overview now surfaces B1 module section, B1 process chart, and exporter health. Exporter has `--dry-run` for safe verification (payload/env health without writes); live export connectivity/write success remains a separate proof surface. B1 v1 Local Website Defect Watcher implemented and proven; fresh proof run 0 findings after signal hardening. Overall verdict: mostly passed with bounded blockers. **2026-03-20:** Dashboard stale Overview data fixed; Overview route uses `fetchCache = "force-no-store"`; live data verified after restart and export refresh; build blocker unchanged.

## New-chat handoff rule (live process)

Every new Jarvis chat must include the **three** attachments called out in **`JARVIS_NEW_CHAT_HANDOFF_BUNDLE_LATEST.md`** (canonical pointer): **`JARVIS_LIVE_HANDOFF_BUNDLE.md`**, **`JARVIS_TASK_EXECUTION_CHECKLIST.md`**, **`JARVIS_HYBRID_OPERATING_DOCTRINE_v1.md`**. You may attach the pointer file plus those three, or attach the three directly.
...
- Did handoff/current-state docs update? (JARVIS_LIVE_HANDOFF_BUNDLE, JARVIS_NEW_CHAT_HANDOFF_BUNDLE_LATEST.md, JARVIS_HYBRID_OPERATING_DOCTRINE_v1.md, JARVIS_PHASE_CHECKLIST, JARVIS_SCRIPT_PROCESS_REFERENCE)
...
**Current status:** Pathfinder v1 is proven. Proof was completed with a successful live run of `run_pathfinder.py` using a realistic WCS intake packet. Pathfinder accepts minimal intake or full task-packet input, gathers bounded context from workspace artifacts and WCS repo files, and produces structured result output with an optional draft backlog candidate or escalation when blocked. Optional LLM synthesis is implemented via `pathfinder_llm_synthesis.py`; when the module is unavailable, the API key is missing, or synthesis fails, Pathfinder safely falls back to rule-based output and records `llm_skipped_reason` in the result. Result output includes `synthesis_source` (rule_based or llm) and, when applicable, `llm_skipped_reason`. CLI: `--no-llm` to skip LLM, `--model` to override. **Fallback + LLM path proven 2026-03-19:** fallback proof: `--no-llm` => `rule_based`/`no_llm`; earlier LLM-enabled failure: `rule_based`/`validation_failure`; current LLM-enabled success: `synthesis_source=llm`. Validation failure diagnostics now preserve specific reason via `validation_failure:<reason>`. It remains manual/CLI-invoked, read-only, and intentionally limited: no code edits, no git actions, no unattended mode, no scheduling, and no broad repo crawl.
