...
* Pathfinder optional LLM synthesis — **fallback path proven**; `run_pathfinder.py` supports `--no-llm` and `--model`; safe fallback to rule-based when module/API key absent or synthesis fails; result includes `synthesis_source` and `llm_skipped_reason`; LLM path and broader expansion still pending
